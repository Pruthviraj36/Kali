...
console.log('18. Check Disk Space');
console.log('19. Check File Permissions');
console.log('20. Check DNS Resolution');
console.log('21. Check SSL Certificate');
console.log('22. Check HTTP Headers');
console.log('23. Check File Hash');
console.log('24. Exit\n');
    ...
    rl.question('What do you want to do: ', async (command) => {
        ...
        if (command == 1) {
            ...
            rl.question('Enter Target IP/Domain: ', (host) => {
                rl.question('Enter Port Range (e.g., 20-100): ', async (range) => {
                    ...
                    console.log(`Scanning ports ${start}-${end} on ${host}...`);
                    await scanPorts(host, start, end);
            ...
            child_process.exec(os.platform() === 'win32' ? 'wmic logicaldisk get size,freespace,caption' : 'df -h', (err, stdout, stderr) => {
                if (err) console.log('Error:', err.message);
                else console.log(stdout);
                askQuestion();
            });
            ...
            rl.question('Enter File Name: ', (fileName) => {
                fs.stat(fileName, (err, stats) => {
                    if (err) console.log('Error:', err.message);
                    else console.log('File Permissions:', stats.mode.toString(8));
                    askQuestion();
                });
            });
        } else if (command == 20) {
            rl.question('Enter Domain: ', (domain) => {
                child_process.exec(`nslookup ${domain}`, (err, stdout, stderr) => {
                    if (err) console.log('Error:', err.message);
                    else console.log(stdout);
                    askQuestion();
                });
            ...
            rl.question('Enter Domain: ', (domain) => {
                child_process.exec(`openssl s_client -connect ${domain}:443 -showcerts`, (err, stdout, stderr) => {
                    if (err) console.log('Error:', err.message);
                    else console.log(stdout);
                    askQuestion();
                });
